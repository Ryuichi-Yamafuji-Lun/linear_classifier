...
def binary_train(X, y, loss="perceptron", w0=None, b0=None, step_size=0.5, max_iterations=1000):
    ...
    if loss == "perceptron":
        ...
        for _ in range(max_iterations):
            ...
            missclassified = (y_transformed * z) <= 0

            # get the updates 
            w_update = np.dot(X[missclassified].T, y_transformed[missclassified]) / N

            b_update = np.sum(y_transformed[missclassified]) / N

            w = w + step_size * w_update
            b = b + step_size * b_update
